...
def load_pdf_files():
    try:
        if not os.path.exists(DATA_PATH):
            raise CustomException("Datapath Doesn't exists..")
        ...
        doccuments = loader.load() # pdf store in this variable
        
        if not doccuments:
            logger.warning("No pdf were found.")
        else:
            logger.info(f"Successfully fetched {len(doccuments)} of documents")
        return doccuments        
